...
const codeFileName = (namespace, codeId) => `${namespace}/${codeId}.js`;
...
const executeFunctionInSandbox = (id, data) => {
  return new Promise((resolve, reject) => {
    ...
    verbose(`Spawn forked sandbox function ${id} of pid: ${childs[id].pid}`);

    childs[id].on('message', (message) => {
      const result = message;
      if (result.error) {
        verbose(`Aborting forked sandbox with error child_process of ${id} of pid: ${childs[id].pid}`);
        reject(result.error);
      }
      
      verbose(`Finishing forked sandbox function ${id} with success of pid: ${childs[id].pid}`);
      resolve(result.data);
      delete childs[id];
    });
  });
