...
class Sandbox {
  ...
  runScript(script, req, { prefix, env, console } = {}) {
      ...
      vmDomain.run(() => {
        ...
        if (result.constructor.name === 'Promise') {
          result
            ...
            .catch((err) => {
              sandboxRes.status(err.statusCode || 500);
              if (err.message) {
                sandboxRes.send({ error: err.message });
              } else {
                sandboxRes.send(null);
              }
            });
          return;
        }

        sandboxRes.send(result);
      });
    });
  }
