...
class Sandbox {
  constructor({ env, globalModules, asyncTimeout, syncTimeout, log, config }) {
    this.context = {
      Backstage: null,
      Buffer: null,
      console: null,
  ...
  createEmptyContext(backstageOptions, prefix = null) {
    const exports = {};
    ...
    const sandboxRequire = new Require(this.context.Backstage.modules, '.', this.globalModules);

    if (prefix) {
      this.context.console = new PrefixLog(prefix, this.log);
    } else {
      this.context.console = this.log;
    }
